...
export default {
  ...
  data() {
    return {
      ...
      rangeSelected: true,
      rangeErrorMsg:
        "You have to select the start date and end date of the project",
      value: [0, 100],
  ...
  methods: {
    ...
    checkDate() {
      if (this.date[0] == null || this.date[1] == null) {
        ...
        console.log(
          this.date[1].toLocaleDateString("en-us", {
            year: "numeric",
            month: "short",
            day: "numeric",
          })
    ...
    includesPrice(pricePropArray, projectPrice) {
      if (
        projectPrice == "" ||
        (parseInt(projectPrice) >= parseInt(pricePropArray[0]) &&
          parseInt(projectPrice) <= parseInt(pricePropArray[1]))
      ) {
        return true;
      } else {
        return false;
    ...
    async submitFilter() {
      ...
      projects.forEach((doc) => {
        ...
        if (
          ...
          if (this.includesPrice(this.value, price)) {
            if (
              this.includesTags(this.interests, projectTags) &&
      ...
      if (this.$route.name == "StudentSearchResult") {
        this.$router.push({
          ...
          params: {
            displayFirst: allMatch,
            displaySecond: noneMatch,
            displayThird: noneMatch,
          },
        });
      } else if (this.$route.name == "StudentSearchResult2") {
        this.$router.push({
