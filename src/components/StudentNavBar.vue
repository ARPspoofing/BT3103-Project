  ...
  <nav class="navbar navbar-expand-lg navbar-custom">
    ...
    >
      <form v-show="search" class="form-inline my-2 my-lg-0">
        <input
          v-show="search"
          class="form-control-inline mr-sm-2"
          type="search"
          placeholder="Search"
          aria-label="Search"
      ...
      <div class="dropdown">
        ...
        <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
          <router-link
            ...
            :to="{ name: 'StudentHomePage' }"
            >Home</router-link
          >
          <div class="dropdown-divider"></div>
          <router-link
            class="nav-item nav-link active"
            :to="{ name: 'StudentAbout' }"
            >Profile</router-link
          >
          <div class="dropdown-divider"></div>
          <router-link
            class="nav-item nav-link active"
            :to="{ name: 'StudentInProgressProjects' }"
            >My Projects</router-link
          >
          <div class="dropdown-divider"></div>
          <router-link
            class="nav-item nav-link active"
            :to="{ name: 'OfferedApplications' }"
...
export default {
  ...
  methods: {
    ...mapMutations([
      ...
      "SET_SECOND_PRIORITYIDS",
      "SET_THIRD_PRIORITYIDS",
      "CLEAR_ALL",
    ]),
    async logOut() {
      const auth = getAuth();
    ...
    includesDate(datePropArray, projectDateArray) {
      if (
        [Date.parse(String(datePropArray[0])) >= projectDateArray[0]] && [
          Date.parse(String(datePropArray[1])) <= projectDateArray[1],
        ]
      ) {
        return true;
      } else {
    ...
    includesPrice(pricePropArray, projectPrice) {
      if (
        projectPrice >= parseInt(pricePropArray[0]) &&
        projectPrice <= parseInt(pricePropArray[1])
      ) {
        return true;
      } else {
        return false;
    ...
    includesTags(tagPropArray, projectTagsArray) {
      ...
      tagPropArray.forEach((tag) =>
        true_arr.push(this.includes(projectTagsArray, tag))
      );
      if (true in true_arr) {
        return true;
      } else {
      ...
      if (/\S/.test(currSearch) && currSearch != "") {
        ...
        projects.forEach((doc) => {
          ...
          if (
            ...
            this.includes(projectTags, searchObj)
          ) {
            matchingResultsByBoth.push(id);
          } else if (
            searchObj.includes(testname) ||
            testname.includes(searchObj)
          ) {
            matchingResultsByTitle.push(id);
          } else if (this.includes(projectTags, searchObj)) {
            matchingResultsByTag.push(id);
          } else if (
            this.includesDate(this.dateVal, [projectStart, projectEnd])
          ) {
        ...
        if (this.$route.name == "StudentSearchResult") {
          this.$router.push({
            ...
            params: {
              ...
              displayThird: matchingResultsByTag,
              displayFourth: matchingResultsByDate,
              displayFifth: matchingResultsByPrice,
              displaySixth: matchingResultsByCompany,
              searched: currSearch,
            },
          });
