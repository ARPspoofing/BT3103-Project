...
export default {
  ...
  methods: {
    async accApplicant(key) {
      ...
      //getOfferedProjects(accApplicant)
  ...
  mounted() {
    ...
    if (this.$route.params.newApplicants) {
      ...
      for (var i = 0; i < this.newApplicants.length; i++) {
        var email = this.newApplicants[i];
        ...
        //console.log(this.newApplicants[i], this.offered)
      }
    }
    ...
    async function getApplicant(app) {
      ...
      return { email: data.email, name: data.name, course: data.course };
    }
